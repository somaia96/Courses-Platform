    ...
    return (
        <div className="flex absolute left-1/2 bottom-16 -translate-x-1/2 gap-3 items-center justify-center">
            {Array.from({ length: Math.ceil(total / 6) }, (_, i) => {
                return <span key={i} onClick={() => setPage(i)} className="cursor-pointer text-white flex justify-center items-center font-semibold p-1 rounded-full w-10 h-10 bg-blue-800">
                    {i + 1}
                </span>
            })}
        </div>
